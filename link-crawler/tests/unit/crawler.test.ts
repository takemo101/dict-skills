...
`;
        ...
        pages: [
          {
            ...
            depth: 0,
            links: [],
            metadata: { title: "Old Title", description: null, keywords: null, author: null, ogTitle: null, ogType: null },
  ...
  describe("pages option", () => {
    it("should not save individual pages when pages is false", async () => {
      ...
      const hasPageFiles = existsSync(pagesDir) && 
        (await readFile(join(testDir, "index.json"), "utf-8"))
          .includes("page-001.md");
